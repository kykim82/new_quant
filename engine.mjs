...
import {PerformanceTracker,seedLegacy,capturePublished,performanceSnapshot} from './recommendation_performance.mjs';
...
export class Radar {
  ...
  async init(){this.stage('저장 이력 불러오기');await ensureSchema(this.db);
    ...
    for(const [id,row] of this.plans)if(seedLegacy(row))this.planDirty.add(id);
  ...
  remember(result){if(!result.plan)return;const id=result.plan.id,old=this.plans.get(id),now=this.clock();
    ...
    const row={...old,market:result.market,name:result.name,unit:result.unit,plan,firstShownAt:old?.firstShownAt??(result.status==='ready'?now:null)};
  ...
  async flush(force=false){if(!this.token)return;const now=this.clock();if(!force&&now-this.lastSave<3000)return;this.stage('분석 결과 DB 저장');await this.renew();
    ...
    for(const code of this.dirty){const cache=this.caches.get(code);if(!cache)continue;
      for(const unit of RECOMMEND_UNITS){if((cache[unit]?.length??0)<=RETAINED_BARS)continue;const history=assessHistory(cache,unit,now);if(!history.ready)continue;
        const bars=rawBars(cache,unit,endOf(unit,now));
        for(const [id,row] of this.plans){if(row.market!==code||row.unit!==unit||row.plan.stoppedAt||row.plan.completedAt)continue;const plan=observePlan(row.plan,bars,this.quotes.get(code),now);if(JSON.stringify(plan)!==JSON.stringify(row.plan)){this.plans.set(id,{...row,plan});this.planDirty.add(id);}}
  ...
  async process(info,phase){const now=this.clock(),state=this.states.get(info.market)??{version:VERSION,market:info.market,frames:{},trends:{}};this.states.set(info.market,state);
    ...
    if(phase==='analysis')await this.warmUpper(info);
  }
  ...
  upperQueue(){const now=this.clock(),jobs=[],rows=this.snapshot().rows.filter(r=>UNITS.includes(r.unit));
    const visible=new Set(UNITS.flatMap(u=>rows.filter(r=>r.unit===u).slice(0,10).map(r=>r.market))),qualified=new Set(rows.map(r=>r.market));
    ...
    return jobs.sort((a,b)=>a.tier-b.tier||a.lastAttempt-b.lastAttempt||b.priority-a.priority||a.unit-b.unit||a.info.market.localeCompare(b.info.market));
  }
  async processUpper(info,unit,{deadline=this.clock()+3000,maxPages=16}={}){const state=this.states.get(info.market);state.upperTracking.attempts??={};state.upperTracking.attempts[unit]=this.clock();
  ...
  async cycle({maxMarkets=Infinity,retainLease=false}={}){this.stage('분석 잠금 확인');if(!await this.acquire()){const row=await this.db.prepare('SELECT payload_json FROM radar_snapshot WHERE id=1').first();if(row&&parse(row.payload_json).stSettings===ST_SETTINGS)this.emit({type:'snapshot',payload:parse(row.payload_json)});this.emit({type:'waiting',message:'다른 실행이 수집한 저장 결과를 확인 중입니다.'});return;}
    ...
    for(const row of restored?.plans??[]){const current=this.plans.get(row.plan.id);if(current&&seedLegacy(current))this.planDirty.add(row.plan.id);}
    ...
    try {this.stage('분석 대상·시세 조회');await this.refreshMarkets();await this.refreshQuotes();await this.flush(true);
      ...
      const upperDeadline=this.clock()+30000;
...
async function main(){const local=process.env.QUANT_LOCAL_DB?await localDatabase(process.env.QUANT_LOCAL_DB):null;
  ...
  const performanceTimer=setInterval(()=>{void radar.performance.tick(radar);},1000);
  const timer=setInterval(async()=>{if(!radar.token||quoteBusy)return;quoteBusy=true;try{await radar.refreshMarkets();await radar.refreshQuotes();radar.beta.observe(radar);radar.captureRecommendations();if(radar.planDirty.size)await radar.flush(true);emit({type:'snapshot',payload:radar.snapshot()});}catch(e){radar.error(e.message);}finally{quoteBusy=false;}},10000);
  ...
  finally{clearInterval(timer);clearInterval(syncTimer);clearInterval(backupTimer);clearInterval(performanceTimer);while(quoteBusy)await sleep(50);await radar.performance.busy;try{await radar.flush(true);await radar.release();}finally{input.close();local?.close();if(buffer)await buffer.close();}}
}
